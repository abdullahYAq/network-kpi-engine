import pandas as pd
from src.db.kpi_counter_values_repository import get_kpi_id_kpi_name_map
from src.config.db_config import db_config
from src.db.cells_repository import get_lncel_name_id_map,get_distname_id_map
import datetime
# === LOADING ===
def load_kpi_values(csv_path):
    #csv_df = 
...
def map_cells_to_distname(df):
    ...
    return df
# === TRANSFORM ===
def rename_kpi_columns_to_ids(kpi_columns,df):
    kpi_map = get_kpi_id_kpi_name_map(db_config)
    skeped_counters = []
    ...
    for kpi in kpi_columns:
        if kpi in kpi_map:
            valid_counters.append(kpi)
            rename_map.update({
                kpi:kpi_map[kpi]
                })
        else:
            skeped_counters.append(kpi) 
...
def transform_wide_to_long(df):
    ...
    long_df = df.melt(id_vars=id_vars,value_vars=value_vars,var_name="kpi_id",value_name="kpi_value")
    long_df["kpi_id"] = pd.to_numeric(long_df["kpi_id"],errors="coerce")
    return long_df
def map_and_rename_column_from_dict(df,column_name):
    ...
    cleaned_df["kpi_value"] = pd.to_numeric(cleaned_df["kpi_value"],errors="coerce",downcast="integer")
